...
protocol JTranslrToLangSelecControllerDelegate: AnyObject {
    func didSelect(language: TranslationLanguage)
}

final class JTranslrToLangSelecController: UITableViewController {


    var languages: [TranslationLanguage] = []

    weak var selectionDelegate: JTranslrToLangSelecControllerDelegate?

    private let reuseIdentifier = "localeCell"

    override func viewDidLoad() {
        super.viewDidLoad()
        let text = String(format: "Available Languages %@".localized(), "\(languages.count)")
        title = text
    }
