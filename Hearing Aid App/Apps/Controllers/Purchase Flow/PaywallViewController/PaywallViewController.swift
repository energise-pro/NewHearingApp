...
final class PaywallViewController: UIViewController {
    ...
    //MARK: - Properties
    private var typeScreen: TypePaywallScreen
    private var scaleYearlyButton = true
    private var isLoading: Bool = false
    private var subscriptionItems: [ShopItem] = []
    private var openAction: AnalyticsAction
    
    //MARK: - Init
    init(typeScreen: TypePaywallScreen, openAction: AnalyticsAction) {
    ...
    private func configureUIAfterSubscriptionsLoading() {
        switch typeScreen {
        case .regular:
            guard let yearlySubscriptionPlan = subscriptionItems.first(where: { $0.productId == Constants.Keys.annualSubscriptionId }),
                  let monthlySubscriptionPlan = subscriptionItems.first(where: { $0.productId == Constants.Keys.monthlySubscriptionId }) else {
                return
            }
    ...
    @IBAction private func monthlySubscribeButtonAction(_ sender: UIButton) {
        guard let subscriptionPlan = subscriptionItems.first(where: { $0.productId == Constants.Keys.monthlySubscriptionId }) else {
            return
        }
