...
final class MicrophonePermissionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        AppConfiguration.shared.analytics.track(.v2MicrophoneReminder, with: [GAppAnalyticActions.action.rawValue: GAppAnalyticActions.open.rawValue])
        configureUI()
    }
    
    //MARK: - IBActions
    @IBAction private func allowButtonAction(_ sender: UIButton) {
        AppConfiguration.shared.analytics.track(.v2MicrophoneReminder, with: [GAppAnalyticActions.action.rawValue: GAppAnalyticActions.allow.rawValue])
    ...
    @IBAction private func latterButtonAction(_ sender: UIButton) {
        AppConfiguration.shared.analytics.track(.v2MicrophoneReminder, with: [GAppAnalyticActions.action.rawValue: GAppAnalyticActions.close.rawValue])
        TapticEngine.impact.feedback(.medium)
