...
final class ProSetupViewController: PMBaseViewController {
    ...
    private func configureDataSource() {
        let avaliableMicrophones = AudioKitService.shared.connectedHeadphones ? MicrophoneType.allCases : [.bottom, .front, .back]
        let segmentCellModel = SimpleSegmentTableViewCellModel(mainTitle: "Active microphone".localized(), titles: avaliableMicrophones.compactMap { $0.title }, selectedIndex: MicrophoneType.selectedMicrophone.rawValue, delegate: self)
...
extension ProSetupViewController: SimpleSegmentTableViewCellDelegate {
    ...
    func didSelectSegment(with index: Int, from cell: UITableViewCell) {
        ...
        AudioKitService.shared.changeMicrophone(on: MicrophoneType(rawValue: index)!)
