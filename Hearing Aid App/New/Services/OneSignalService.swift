...
final class OneSignalService: NSObject, IServiceProtocol {
    ...
    private func configureCatchUpPushNotification() {
        ...
        notificationContent.userInfo["custom"] = ["a": ["open": Constants.Keys.catchUpScreenName]]
        notificationContent.sound = UNNotificationSound.default

        let notificationCenter = UNUserNotificationCenter.current()
        
        let trigger = UNCalendarNotificationTrigger(dateMatching: dateComponent, repeats: true)
        let request = UNNotificationRequest(identifier: Constants.Keys.catchUpScreenName, content: notificationContent, trigger: trigger)
    ...
    private func openScreen(with screenName: String, with openAction: GAppAnalyticActions) {
        ...
        case Constants.Keys.paywallScreenName:
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                NavigationManager.shared.presentPaywallViewController(with: openAction)
            }
        case Constants.Keys.catchUpScreenName:
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
