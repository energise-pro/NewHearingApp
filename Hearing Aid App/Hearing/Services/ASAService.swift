...
final class ASAService {
    ...
    private func setAppInstall(searchAdAttributes: [String: Any]) {
        ...
        jsonFields["_api_key"] = CAppConstants.General.APIKey
        
        let url = URL(string: CAppConstants.General.baseURL + "app_installs/register")!
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            UserDefaults.standard.setValue(true, forKey: CAppConstants.Keys.wasSentASAInstall)
    ...
    private func setAppSearchAdsInfo(searchAdAttributes: [String: Any]) {
        ...
        // create post request
        let url = URL(string: CAppConstants.General.baseURL +  "users/add_search_ads_info")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
