...
typealias TranslateServiceCompletion = (String?) -> Void
typealias TranslateServiceDownloadCompletion = (Bool) -> Void

typealias TranslationLanguage = TranslateLanguage

final class TranslateService: NSObject {
    
    static let shared: TranslateService = TranslateService()
    ...
    var inputLocale: Locale? {
        return TranscribeService.shared.supportedLocales.first(where: { $0.identifier.components(separatedBy: "-").first == inputLanguage.rawValue })
    }
    ...
    var inputLanguages: [TranslateLanguage] {
        let filteredLanguages = TranslateLanguage.allLanguages().filter { translateLanguage in
            return TranscribeService.shared.supportedLocales.contains(where: { $0.identifier.components(separatedBy: "-").first == translateLanguage.rawValue })
        }
    ...
    private var downloadCompletion: TranslateServiceDownloadCompletion?
    ...
    func changeInputLanguage(on language: TranslationLanguage) {
        ...
        if let inputLocale = TranscribeService.shared.supportedLocales.first(where: { $0.identifier.components(separatedBy: "-").first == language.rawValue }) {
            TranscribeService.shared.stopRecognition()
            TranscribeService.shared.chargeRecognation(on: inputLocale)
        }
    }
    ...
    func downloadModel(language: TranslateLanguage, completion: TranslateServiceDownloadCompletion?) {
