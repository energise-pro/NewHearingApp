...
final class AppConfiguration: NSObject, IServiceProtocol {
    ...
    // MARK: - Methods
    override init() {
        let inAppPurchasesService = InAppPurchasesService(apiKey: CAppConstants.General.appHudKey)
        oneSignalService = OneSignalService(apiKey: CAppConstants.General.oneSignalKey)
        let services: [IServiceProtocol] = [inAppPurchasesService, FirebaseService(), oneSignalService, AmplitudeService(apiKey: CAppConstants.General.amplitudeKey), BranchService(), FacebookService()]
