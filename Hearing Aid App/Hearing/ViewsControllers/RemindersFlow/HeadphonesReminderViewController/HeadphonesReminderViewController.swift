...
final class HeadphonesReminderViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        AppConfiguration.shared.analytics.track(.v2HeadphonesReminder, with: [GAppAnalyticActions.action.rawValue: GAppAnalyticActions.open.rawValue])
        configureUI()
        configureObserver()
        chargePlayer()
    }
    ...
    @IBAction private func nextButtonAction(_ sender: UIButton) {
        AppConfiguration.shared.analytics.track(.v2HeadphonesReminder, with: [GAppAnalyticActions.action.rawValue: GAppAnalyticActions.close.rawValue])
        TapticEngine.impact.feedback(.medium)
        dismiss(animated: true)
    }
    
    @IBAction private func airplayButtonAction(_ sender: UIButton) {
        AppConfiguration.shared.analytics.track(.v2HeadphonesReminder, with: [GAppAnalyticActions.action.rawValue: GAppAnalyticActions.airPlay.rawValue])
        TapticEngine.impact.feedback(.medium)
    ...
    @objc private func audioRouteChanged(notification: NSNotification) {
        ...
        case AVAudioSession.RouteChangeReason.newDeviceAvailable.rawValue:
            AppConfiguration.shared.analytics.track(.v2HeadphonesReminder, with: [GAppAnalyticActions.action.rawValue: GAppAnalyticActions.connected.rawValue])
