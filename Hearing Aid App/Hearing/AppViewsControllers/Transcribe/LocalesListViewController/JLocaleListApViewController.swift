...
final class JLocaleListApViewController: PMUMainViewController {
    ...
    private func configureUI() {
        ...
        case .translateTo:
            titleText = "Translate language".localized()
        case .translateFrom:
            titleText = "Your language".localized()
        }
        title = titleText
        ...
        let cellNibs: [UIViewCellNib.Type] = [SettingTableViewCell.self, LocaleListTableViewCell.self]
        ...
        case .translateTo:
            ...
            self.dataSource = filteredLanguages
                .compactMap {
                    ...
                    let subTitle = isDownloadingModel ? "Downloading".localized() : (isDownloadedModel ? "" : "Download a language pack for translation".localized())
                    let title = Locale.current.localizedString(forLanguageCode: $0.rawValue)?.capitalized ?? ""
                    let titleAttributedString = NSMutableAttributedString()
                    let buttonTypes: [LocaleListTableViewButtonType] = isDownloadingModel ? [.loader] : [.rightButton]
                    titleAttributedString.append(NSAttributedString(string: title, attributes: [.font: UIFont.systemFont(ofSize: 17.0, weight: .regular), .foregroundColor: UIColor.appColor(.Purple100)!]))
                    return LocaleListTableViewCellModel(
                        attributedTitle: titleAttributedString,
                        subtitle: subTitle,
                        buttonTypes: buttonTypes,
                        topInset: 10,
                        rightImage: isSelected ? UIImage(named: "checkBoldIcon") : UIImage(named: isDownloadedModel ? "" : "downloadCloudIcon"),
                        delegate: self
                    )
                }
                .compactMap { LocaleListTableViewCellConfig(item: $0) }
        case .translateFrom:
            ...
            self.dataSource = filteredLanguages
                .compactMap {
                    ...
                    let subTitle = isDownloadingModel ? "Downloading".localized() : (isDownloadedModel ? "".localized() : "Download a language pack for translation".localized())
                    let title = Locale.current.localizedString(forLanguageCode: $0.rawValue)?.capitalized ?? ""
                    let titleAttributedString = NSMutableAttributedString()
                    let buttonTypes: [LocaleListTableViewButtonType] = isDownloadingModel ? [.loader] : [.rightButton]
                    titleAttributedString.append(NSAttributedString(string: title, attributes: [.font: UIFont.systemFont(ofSize: 17.0, weight: .regular), .foregroundColor: UIColor.appColor(.Purple100)!]))
...
extension JLocaleListApViewController: LocaleListTableViewCellDelegate {
