...
final class TPermissListApViewController: PMUMainViewController {
    ...
    private func autoCloseIfNeeded() {
        ...
        let microPermissionGranted = SAudioKitServicesAp.shared.recordPermission == .granted
    ...
    private func updateAllowButtons() {
        allowButtons[PermissionType.micro.rawValue].setTitle(SAudioKitServicesAp.shared.recordPermission == .granted ? "Allowed".localized() : "Allow".localized(), for: .normal)
        allowButtons[PermissionType.speechRecognition.rawValue].setTitle(CTranscribServicesAp.shared.recordPermission == .authorized ? "Allowed".localized() : "Allow".localized(), for: .normal)
        
        allowButtons[PermissionType.micro.rawValue].backgroundColor = SAudioKitServicesAp.shared.recordPermission == .granted ? .red : UIColor.appColor(.UnactiveButton_1)
        allowButtons[PermissionType.speechRecognition.rawValue].backgroundColor = CTranscribServicesAp.shared.recordPermission == .authorized ? .red : UIColor.appColor(.UnactiveButton_1)
    }
