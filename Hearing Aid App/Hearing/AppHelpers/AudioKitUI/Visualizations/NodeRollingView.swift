...
public class RollingViewData {
    ...
    var history = [Float](repeating: 0.0, count: 256)
    var framesToRMS = 128
...
public struct NodeRollingView: ViewRepresentable {
    ...
    public var nodeTap: RawDataTap
    
    var rollingData = RollingViewData()

    public init(_ node: Node, color: Color = .gray, backgroundColor: Color = .clear, bufferSize: Int = 1024) {
        metalFragment = FragmentBuilder(foregroundColor: color.cg, backgroundColor: backgroundColor.cg, isCentered: true, isFilled: true)
