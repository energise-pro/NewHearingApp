...
/// Defines the CAppConstants used in the library
public struct MMPPulseCAppConstants {
    // MARK: - Layer names
...
extension MMPPulseLayer {
    ...
    func startAnimation() {
        ...
        self.animationLayer.add(self.animationGroup, forKey: MMPPulseCAppConstants.AnimKeys.pulseAnimation)
    }
    
    /// Stops the animation
    func stopAnimation() {
        self.animationLayer.removeAnimation(forKey: MMPPulseCAppConstants.AnimKeys.pulseAnimation)
    }
}
...
extension MMPPulseLayer {
    ...
    fileprivate func configureAnimationGroup() {
        ...
        let scaleAnimation          = CABasicAnimation(keyPath: MMPPulseCAppConstants.Animations.scale)
        scaleAnimation.fromValue    = 0.0
        scaleAnimation.toValue      = 1.0
        scaleAnimation.duration     = self.animationDuration
        //scaleAnimation.autoreverses = true
        
        let opacityAnimation        = CAKeyframeAnimation(keyPath: MMPPulseCAppConstants.Animations.opacity)
...
public extension UIView {
    ...
    func startPulse(withType type : MMPPulseType, color : UIColor = UIColor.black, pulseCount : Int = 1, repeatCount : Float = .greatestFiniteMagnitude, frequency : TimeInterval = 1, radius : CGFloat = 50, andDuration duration : TimeInterval = 1.0) {
        ...
        outerLayer.name              = MMPPulseCAppConstants.Layers.outerLayer
    ...
    func getOuterLayer() -> MMPPulseLayer? {
        ...
        if let subLayers = self.layer.superlayer?.sublayers {
            for sublayer in subLayers {
                if sublayer.name == MMPPulseCAppConstants.Layers.outerLayer {
